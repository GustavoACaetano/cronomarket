...
<template>
    ...
    >
        <div class="mb-10 gap-y-6 transition-all duration-700 delay-150" :class="isVisible ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'">
            <p class="text-center mb-10 text-3xl">Parabéns, você ganhou</p>
            <h1 class="text-center mb-18 text-9xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-red-500 via-yellow-500 to-red-500 scale-y-125 -skew-y-6 scale-x-150 transform">
                R${{ valorPremio }}!
            </h1>
            <p class="text-center text-lg">
                Disponível agora para você usar no Cronomarket!
            </p>
        </div>
        <div class="mb-24 pt-18 transition-all duration-700 delay-300" :class="isVisible ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'">
